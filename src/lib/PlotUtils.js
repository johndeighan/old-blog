...
export var getPlotOptions = (title, hSeries, hX, hY, hY2 = undef) => {
  var hDesc, hOptions, key, lAllData, lSeries, str;
  ...
  lAllData = [];
  for (key in hSeries) {
    if (!hasProp.call(hSeries, key)) continue;
    hDesc = hSeries[key];
  ...
  if (str = hX.format) {
    if (str === 'date') {
      ...
      hOptions.axes.xaxis.tickOptions = {
        formatString: str
      };
    }
  }
  if (str = hY.format) {
    if (str === 'date') {
      ...
      hOptions.axes.yaxis.tickOptions = {
        formatString: str
      };
    }
  }
  if (defined(hY2)) {
    ...
    if (str = hY2.format) {
      if (str === 'date') {
        ...
        hOptions.axes.y2axis.tickOptions = {
          formatString: str
        };
      }
    }
